...
export function Combobox({ label, placeholder, value, onSelect, options = [], error, disabled, typeable = true, id }) {
  ...
  const filtered = !typeable
    ? options
    : query.trim()
      ? options.filter((o) => o.name.toLowerCase().includes(query.trim().toLowerCase()))
      : options;

  // reset highlight when options/filter changes or dropdown closes
  useEffect(() => { setHighlightedIndex(-1); }, [filtered.length, query, typeable]);
      ...
      <div className="relative">
        ...
        {open && !disabled && filtered.length > 0 && (
          <ul
            ...
            ref={listRef}
            role="listbox"
            className="absolute z-30 left-0 right-0 mt-1 bg-white border border-black/12 rounded-lg shadow-[0_8px_24px_rgba(0,0,0,0.08)] max-h-[220px] overflow-auto"
          >
            {filtered.map((o, idx) => (
              <li
                key={o.code}
