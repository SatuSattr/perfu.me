...
export function VariantsSection({ options, onChange, type, className = '' }: Props) {
    const [panelMode, setPanelMode] = React.useState<'create' | 'edit' | null>(null);
    ...
    function handleSave(draft: OptionItem) {
        ...
        const dupIdx = options.findIndex((o, i) => o.key === draft.key && i !== editingIndex);
        if (dupIdx !== -1) {
        ...
        if (panelMode === 'create') {
            const next = renumber([...options, { ...draft, position: options.length }]);
            onChange(next);
        } else if (panelMode === 'edit' && editingIndex !== null) {
            const merged = options.map((o, i) => (i === editingIndex ? { ...draft, position: i } : o));
            onChange(renumber(merged));
        }
        doClosePanel();
    }
    ...
    const editingOption = editingIndex !== null ? options[editingIndex] ?? null : null;
            ...
            <>
                ...
                >
                    <span className="flex h-10 w-10 items-center justify-center rounded-full bg-white border border-[#e6e6e6] group-hover:bg-[#1a1a1a] group-hover:border-[#1a1a1a] group-hover:text-white transition-colors duration-200">
                        <Plus size={16} strokeWidth={1.8} className="text-[#bbb] group-hover:text-white transition-colors" />
                    </span>
                    <span className="font-sans text-[12px] font-medium text-[#888] group-hover:text-[#1a1a1a] transition-colors">Belum ada varian</span>
                    <span className="font-sans text-[11px] text-[#aaa] group-hover:text-[#666] transition-colors">
                        Klik untuk tambah varian — tiap varian akan jadi panel terpisah
                    </span>
                    <span className="mt-1 font-sans text-[10px] tracking-[0.12em] uppercase text-[#bbb] group-hover:text-[#1a1a1a] opacity-0 group-hover:opacity-100 transition-all duration-200">
                        Tambah Varian →
                    </span>
                </button>
                ...
                <SidePanel
                    ...
                    subtitle={panelMode === 'edit' ? editingOption?.label || editingOption?.key : 'Varian baru akan jadi panel terpisah'}
                    width="md"
                >
                    {panelMode === 'create' ? (
                        <VariantFormPanel mode="create" onClose={requestClosePanel} onSave={handleSave} onDirtyChange={setPanelDirty} />
        ...
        <>
                ...
                {options.map((opt, idx) => (
                    <VariantPanel
                        ...
                        onChoicesChange={(nextChoices) => {
                            const next = options.map((o, i) => (i === idx ? { ...o, choices: nextChoices } : o));
                            onChange(next.map((o, i) => ({ ...o, position: i, choices: o.choices.map((c, ci) => ({ ...c, position: ci })) })));
                        }}
                    />
                ))}
            </div>
            ...
            >
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-white border border-[#e6e6e6] group-hover:bg-[#1a1a1a] group-hover:border-[#1a1a1a] transition-colors duration-200">
                    <Plus size={13} strokeWidth={1.8} className="text-[#aaa] group-hover:text-white transition-colors" />
                </span>
