    ...
    return (
        <div className="flex flex-col">
            ...
            <textarea
                ...
                aria-invalid={!!error}
                aria-describedby={errorId}
                className={`w-full border rounded-lg px-4 py-3 text-[13px] font-sans text-[#111] outline-none transition-colors duration-200 bg-white placeholder:text-[#bbb] resize-none ${error ? 'border-red-400 focus:border-red-400' : 'border-gray-200 focus:border-[#111]'} ${className}`}
                {...props}
            />
            {error && (
                <span id={errorId} className="mt-1.5 font-sans text-[11px] text-red-500" role="alert">
...
export function SearchInput({ value, onChange, placeholder = 'Cari produk...', error, id, ...props }: SearchInputProps) {
