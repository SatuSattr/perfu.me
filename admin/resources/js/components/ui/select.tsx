...
export function Select({ label, required, error, children, placeholder = 'Pilih...', ...props }: SelectProps) {
    return (
        <div className="flex flex-col gap-2">
            ...
            <div className="relative">
                <select className={`w-full border rounded-lg px-4 py-3 pr-9 text-[13px] font-sans text-[#111] outline-none focus:border-[#111] transition-colors duration-200 bg-white appearance-none cursor-pointer ${error ? 'border-red-400' : 'border-gray-200'}`} {...props}>
...
export function ChoiceGroup({ label, required, options = [], value, onChange, error }: ChoiceGroupProps) {
    return (
        <div className="flex flex-col gap-2">
            {label && (
                <label className="font-sans text-[10px] font-semibold uppercase tracking-[0.16em] text-[#555]">
            ...
            <div className="flex flex-wrap gap-2">
                {options.map((choice) => {
                    ...
                    const isDisabled = (choice.stock ?? 1) <= 0;
                    return (
                        <button
                            key={choice.id}
                            type="button"
